...
void loop()
{
  ...
  hal.rcout->write(MOTOR_BL, 1.1 * rcthr);
  ...
  if(rcthr > 1088) 
  { // *** MINIMUM THROTTLE TO DO CORRECTIONS MAKE THIS
    //20pts ABOVE YOUR MIN THR STICK ***/
    
    // Stab PIDS
    float pitch_stab_output = constrain(pids[PID_PITCH_STAB].get_pid((float)rcpit
    - pitch, 1), -250, 250);
    ...
    constrain(pids[PID_YAW_STAB].get_pid(wrap_180(yaw_target - yaw), 1), -360,
    360);
    ...
    constrain(pids[PID_PITCH_RATE].get_pid(pitch_stab_output - gyroPitch, 1), -
    500, 500);
    long roll_output = (long)
    constrain(pids[PID_ROLL_RATE].get_pid(roll_stab_output - gyroRoll, 1), -500,
    500);
    long yaw_output = (long)
    constrain(pids[PID_YAW_RATE].get_pid(yaw_stab_output - gyroYaw, 1), -500,
    500);
